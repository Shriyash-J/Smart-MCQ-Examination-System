...
const QuestionForm = ({ question, index, onChange, onRemove, showRemove }) => {
  ...
  const handleDiagramUpload = async (e) => {
    ...
    // For development, create a local object URL for preview
    ...
    <div className="card mb-6 relative">
      ...
      <div className="mb-4">
        ...
        <textarea
          ...
          onChange={(e) => handleTextChange('questionText', e.target.value)}
          className="w-full p-2 border rounded"
          rows="2"
            ...
            <div className="space-y-2">
              {['True', 'False'].map((opt, optIndex) => (
                <div key={optIndex} className="flex items-center">
                  <input
                    type={
                      question.questionType === 'msq' ? 'checkbox' : 'radio'
                    }
                    name={`correct-${index}`}
                    checked={
                      question.correctAnswers?.includes(optIndex) || false
                    }
                    onChange={(e) =>
      ...
      {question.questionType === 'manual' && (
        <div className="mb-4">
          ...
          <textarea
            ...
            onChange={(e) => handleTextChange('expectedAnswer', e.target.value)}
            className="w-full p-2 border rounded"
            rows="3"
      ...
      <div className="w-32">
        ...
        <input
          ...
          onChange={(e) =>
            handleTextChange('marks', parseFloat(e.target.value) || 1)
          }
          min="0.5"
          step="0.5"
          className="w-full p-2 border rounded"
      ...
      {/* Partial Marking for MSQ */}
      {question.questionType === 'msq' && (
        <div className="mt-3">
          <label className="flex items-center">
            <input
              type="checkbox"
              checked={question.partialMarking !== false}
              onChange={(e) =>
                handleTextChange('partialMarking', e.target.checked)
              }
              className="mr-2"
            />
            <span className="text-sm">Enable partial marking</span>
          </label>
        </div>
      )}
    </div>
